			...
			<h1>Cuentas</h1>
					...
					if(isset($_POST['enviar'])){
						...
						$stmt = $conexion->prepare("SELECT * FROM usuarios
													WHERE usuario_nombre LIKE :busqueda OR
													usuario_apellido LIKE :busqueda OR
													usuario_username LIKE :busqueda");
				    ...
				    $listaPacientes = $conexion->query("SELECT * FROM usuarios 
				                                        ORDER BY usuario_apellido $sort_order")
				                               ->fetchAll();
					}
					?>
		...
		<button class="sortbutton" onclick="sortZA()"><span class="material-symbols-outlined">unfold_more</span>Z-A</button>
			...
			<table class="tablaDePacientes">
				...
				<?php foreach ($listaPacientes as $datos) {
					$nombre = urlencode($datos['usuario_nombre']);
					$apellido = urlencode($datos['usuario_apellido']);
					$email = urlencode($datos['usuario_email']);
					echo'
					<tr>'.
						'<td data-cell="Nombre"> <span>'.$datos['usuario_nombre'].'</span> </td>'.
						'<td data-cell="Apellido"> <span>'.$datos['usuario_apellido'].'</span> </td>'.
						'<td data-cell="Apellido"> <span>'.$datos['usuario_email'].'</span> </td>'.
					...
					'<td data-cell="Editar">
						...
						<form action="./index.php?vista=edit" method="POST">
							...
							<input type="hidden" name="clave" value='.$datos['usuario_clave'].'>
							<input type="hidden" name="page" value="listaDeCuentas">
